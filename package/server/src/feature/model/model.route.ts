...
export const modelRoute = async (app: FastifyTypebox) => {
	...
	app.route({
		method: 'post',
		url: '/ewe/UploadTimeseries',
		schema: {},
		handler: async (req, res) => {
			try {
				res.status(200).send(await eweService.UploadTimeseries(req, res))
			} catch (error) {
	...
	app.route({
		method: 'post',
		url: '/ewe/UploadForcing',
		schema: {},
		handler: async (req, res) => {
			try {
				res.status(200).send(await eweService.UploadForcing(req, res))
			} catch (error) {
				if (error instanceof Error) {
					res.status(200).send({ status: 'fail', content: error.message })
				} else;
	...
	app.route({
		method: 'post',
		url: '/ewe/UploadMeasured',
		schema: {},
		handler: async (req, res) => {
			try {
				res.status(200).send(await eweService.UploadMeasured(req, res))
			} catch (error) {
				if (error instanceof Error) {
					res.status(200).send({ status: 'fail', content: error.message })
				} else;
	...
	app.route({
		...
		handler: async (req, res) => {
			try {
				res.status(200).send(await eweService.Run_Model(req, res))
			} catch (error) {
				if (error instanceof Error) {
					res.status(200).send({ status: 'fail', content: error.message })
				} else;
